...
* [2 Правила написания Python-кода](#s2-python-language-rules)
  ...
  * [2.20 Современный Python: Python 3 и from ](#s2.20-modern-python)**[future](#s2.20-modern-python)**[ imports](#s2.20-modern-python)
...
### 3.12 Комментарии TODO, FIXME

Используйте комментарии:

* `TODO` для кода, который является временным,
  краткосрочным решением или достаточно хорошим, но не идеальным.
* `FIXME` для кода, который требуется исправить в след. итерации доработки и в качестве тех. долга.

Комментарий `TODO/FIXME` должен начинаться со строки `TODO/FIXME` заглавными буквами.<br>
Далее следует объяснение того, что нужно делать.

Цель состоит в том, чтобы иметь согласованный формат `TODO/FIXME`, в котором можно
было бы выполнять поиск, чтобы узнать, как получить более подробную информацию.
`TODO/FIXME` здесь скорее является пометкой, в каком месте кода следует вносить
...
В ходе устранения\\доработки указанной в  `TODO/FIXME` комментарий должен быть удален.

```python
# TODO: Используйте "*" для повторения строк.
# FIXME: Необходимо произвести рефакторинг, в дальнейшем использовать pandas для формирования excel.
```
...
Для автоматического форматирования импортов используйте оптимизацию импортов PyCharm: **Code | Optimize import**
...
Вы не можете поместить результат теста в ту же строку, что и тест, даже
если весь оператор умещается в одной строке. В том числе, вы никогда не сможете
сделать это с `try` /` except`, поскольку `try` и` except` не могут
одновременно уместиться в одной строке. Все обусловленно читабельностью кода, а не его компактностью.
...
* Поместите связанные классы и функции верхнего уровня вместе в модуль.
  ...
  модуль.
* Используйте CapWords для имен классов, но lower_with_under.py для имен
...
<table border="1" summary="Guidelines from Guido's Recommendations">
...
* Пропишу явное, данный пункт относится только для Py3
...
def my_method(
    ...
    first_var: int,
    second_var: Foo,
    third_var: Optional[Bar] # Или Bar | None в python 3.11 стиль был изменен.
) -> int:
  ...
```
...
используйте пробелы вокруг `=` *только* для аргументов, которые имеют как
...
если есть только один другой тип, используйте `Optional` или `|` для python \>=3.11.
...
def func(a: Optional[Text], b: Optional[Text] = None) -> Text:
  ...
def func_py311(a: Text | None, b: Text | None) -> Text:
  ...
```
...
def multiple_nullable_union(a: Union[None, Text, int]) -> Text
  ...
def multiple_nullable_union_py311(a: None | Text | int]) -> Text
  ...
```
